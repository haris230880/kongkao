...
class _OrderScreenSaleState extends State<OrderScreenSale> {
  ...
  Future<void> readlistorder() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (result != null) {
          setState(() {
            exchangemodels = List<ExchangeModel>.from(
                result.map((map) => ExchangeModel.fromJson(map)));
          });
        } else {
          normaDiolog(context, 'ไม่พบข้อมูล');
        }
      } else {
        normaDiolog(context, 'เกิดข้อผิดพลาด');
      }
    } catch (e) {
      normaDiolog(context, 'เกิดข้อผิดพลาด: $e');
    }
  }
  ...
  void navigateToOrderStatusPage(ExchangeModel exchangeModel) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => OrderStatusPage(exchangeModel: exchangeModel),
      ),
    );
  }
          ...
          itemBuilder: (context, index) {
            ...
            if (exchangemodels[index].status != "สำเร็จ" && exchangemodels[index].status != "ยกเลิก") {
              return  GestureDetector(
                onTap: () => navigateToOrderStatusPage(exchangemodels[index]),
                child: Card(
                  child: Row(
                    children: [
                      Expanded(
